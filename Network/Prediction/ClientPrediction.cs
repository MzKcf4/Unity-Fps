...
/*
 This class is responsible for the local client's behaviour. It does two things:
 - Record the input block
 - Predict the outcome of said input

 At every tick, the component will check if any new states have been received by the server. 
 If so, the client will be set to that new state, 
 and all recorded input packets from that point onwards will be re-simulated. 
 This fulfills the client-side prediction.
 */
